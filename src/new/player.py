...
import numpy as np
import outputs as lo


class Player:

    def __init__(self):
        self._output_interface = lo.MIDIPlayer()
        self._tempo = le.Tempo(qpm=120)
        self._message_queue = le.LOERICQueue()
        self._lookahead_eighths = 1
        ...
        while not self._message_queue.is_empty():
            ...
            if e.time > tick:
                ...
                if isinstance(e, le.Tempo):
                    self._tempo = e
                elif isinstance(e, le.Lookahead):
                    self._lookahead_eighths = max(
                        e.duration.eighth_duration, self._lookahead_eighths
                    )
    ...
    def set_lookahead(self, lookahead):
        self._lookahead_eighths = lookahead
    ...
    def step(self, mapper, contour_manager, groover, tune, tick):
        ...
        performance_tick = tick + self._lookahead_eighths
    ...
    @property
    def lookahead(self):
        return self._lookahead_eighths
