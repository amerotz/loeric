...
class GateLayer(LOERICMatrix):
    ...
    def reset(self):
        self._matrix = np.eye(len(self._inputs), len(self._inputs))
        self._bias = np.zeros(len(self._inputs))
...
class Mapper:
    ...
    def _preprocess_inputs(self):
        ...
        for i, val in enumerate(inputs):
            self._gate_layer.set(self._input_names[i], not np.isnan(val))
        ...
        # filter
        out = self._gate_layer(out)

        return out

    @property
    def _input_values(self):
        return np.array([self._parameters[n].get() for n in self._input_names])

    @property
    def _impact_values(self):
        return np.array([self._parameters[n].get() for n in self._impact_names])
    ...
    def get(self, as_array=False):
        if as_array:
            return np.array([self._parameters[p].get() for p in self._parameters])
        else:
            return {p: self._parameters[p].get() for p in self._parameters}
